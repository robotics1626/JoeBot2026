...
public class RobotContainer {
    ...
    private void configureButtonBindings() {
        ...
        /// Operator
        operator.b()
                .toggleOnTrue(indexer.index().alongWith(intake.intake()));
                
        operator.rightTrigger()
                .toggleOnTrue(shooter.shoot(6000));

        operator.rightBumper()
                .toggleOnTrue(indexer.feed());

        operator.povUp()
                .onTrue(shooter.stepShroud(2));

        operator.povDown()
                .onTrue(shooter.stepShroud(-2));

        operator.povLeft()
                .onTrue(shooter.shroud(Positions.kDown));

        operator.povRight()
                .onTrue(shooter.shroud(Positions.kUp));
    }
